@file:Suppress("unused")

package hope.base.dhl.plugin

object AndroidX {
    const val appcompat = "androidx.appcompat:appcompat:1.3.0-alpha01"
    const val coreKtx = "androidx.core:core-ktx:1.3.0"
    const val core = "androidx.core:core:1.3.0"
    const val coreAnimation = "androidx.core:core-animation:1.0.0-alpha01"
    const val annotation = "androidx.annotation:annotation:1.1.0"
    const val material = "com.google.android.material:material:1.3.0-alpha01"
    const val recyclerview = "androidx.recyclerview:recyclerview:1.2.0-alpha04"
    const val vectordrawable = "androidx.vectordrawable:vectordrawable:1.1.0"
    const val vectordrawableAnimated = "androidx.vectordrawable:vectordrawable-animated:1.1.0"
    const val constraintlayout = "androidx.constraintlayout:constraintlayout:1.1.3"
    const val constraintlayoutSolver = "androidx.constraintlayout:constraintlayout-solver:1.1.3"
    const val lr = "androidx.lifecycle:lifecycle-runtime:2.2.0"
    const val lcj8 = "androidx.lifecycle:lifecycle-common-java8:2.2.0"
    const val lc = "androidx.lifecycle:lifecycle-common:2.2.0"
    const val le = "androidx.lifecycle:lifecycle-extensions:2.2.0"
    const val ck = "androidx.collection:collection-ktx:1.1.0"
    const val start = "androidx.startup:startup-runtime:1.0.0-alpha01"
}

object OkHttp {
    const val okhttp = "com.squareup.okhttp3:okhttp:4.7.2"
    const val mockwebserver = "com.squareup.okhttp3:mockwebserver:4.7.2"
    const val logging = "com.squareup.okhttp3:logging-interceptor:4.7.2"
    const val retrofit = "com.squareup.retrofit2:retrofit:2.9.0"
    const val gson = "com.squareup.retrofit2:converter-gson:2.9.0"
}

object Fresco {
    const val fresco = "com.facebook.fresco:fresco:2.2.0"
}

object Refresh {
    const val kernel = "com.scwang.smart:refresh-layout-kernel:2.0.1"
    const val header = "com.scwang.smart:refresh-header-material:2.0.1"
}

object Kotlin {
    const val stdlib8 = "org.jetbrains.kotlin:kotlin-stdlib-jdk8:1.3.72"
    const val coroutinesCore = "org.jetbrains.kotlinx:kotlinx-coroutines-core:1.3.7"
    const val coroutinesAndroid = "org.jetbrains.kotlinx:kotlinx-coroutines-android:1.3.7"
}

object Test {
    const val junit = "junit:junit:4.13"
    const val junitAndroidX = "androidx.test.ext:junit:1.1.1"
    const val mockito = "org.mockito:mockito-core:3.3.3"
    const val annotation = "androidx.annotation:annotation:1.1.0"
    const val runner = "androidx.test:runner:1.2.0"
    const val rules = "androidx.test:rules:1.2.0"
    const val espresso = "androidx.test.espresso:espresso-core:3.2.0"
    const val coroutines = "org.jetbrains.kotlinx:kotlinx-coroutines-test:1.3.3"
}